...
func getRoleDescription(Role string) string {
	prompt, err := services.ReadPromptFromFile("prompts/Roles/" + Role)

	if err != nil {
		pkg.DisplayContext("Could not find requested role: "+Role, pkg.Error)
		return ""
	}

	pkg.DisplayContext("Role: "+Role+" has been attributed", pkg.Debug)
	return prompt
}

// interpretLLMDecision interprets a decision string generated by the LLM and dispatches it to the appropriate handler.
func interpretLLMDecision(ctx context.Context, Phase string, Checksum string, GamePrompt string) (string, error) {
	role, err := services.GetEntityRoleByChecksum(Checksum)
	...
	switch Phase {
	case "Discussion":
		return decisions.HandleTalkToLogic(ctx, Checksum, GamePrompt, roleDescription)
	case "Voting":
		return decisions.HandleVotingLogic(ctx, Checksum, GamePrompt, roleDescription)
	case "Night":
		if role != "Werewolf" {
...
func askLLMForDecision(ctx context.Context, Message string, Checksum string) (string, error) {
	...
	decision, err := services.GptSimpleRequest(ctx, Message, systemPrompt)
	if err != nil {
...
func shouldMakeDecision(ctx context.Context, Message string, Checksum string, GamePrompt string) (bool, error) {
	discussion, err := helpers.GetAllDiscussions(Checksum)
	if err != nil {
		return false, err
	}
	...
	response, err := services.GptSimpleRequest(ctx, Message, systemPrompt)
	if err != nil {
		pkg.DisplayContext("GptSimpleRequest failed:", pkg.Error, err)
		return false, err
	}
...
func MakeDecisionService(ctx context.Context, EnvironmentData string, Checksum string, Token string) (map[string]string, error) {
	...
	task, err := interpretLLMDecision(ctx, data["phase"], Checksum, gamePrompt)
	if err != nil {
