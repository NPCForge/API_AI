<template>
    <div style="height: 100%; width: 100%;">
        <h3>Simulation route "Disconnect"</h3>
        <div class="hb"></div>
        <div class="d-flex justify-content-center align-items-start rounded"
            ...
            <div style="width: 100%; padding: 1%;" class="d-flex flex-column justify-content-start align-items-start">
                ...
                <input type="text" placeholder="Action" disabled value="Disconnect" name="Action">
                <label for="Token">Token</label>
                <input type="text" placeholder="Token" name="Token" v-model="token" />
            </div>
...
<script setup>
    ...
    const token = ref("")  // Variable pour stocker le token
    const res = ref("")  // Résultat de la requête

    // Initialiser la valeur du token depuis le localStorage ou une autre source
    onMounted(() => {
        try {
    ...
    // Fonction pour lancer la requête HTTP
    const RunHttp = async () => {
        try {
            isLoading.value = true
            LoadingMessage.value = "Running Http Request"
            res.value = await disconnect(token.value)
        } catch (e) {
            console.error(e)
        } finally {
    ...
    // Formatage du JSON avec une indentation de 4 espaces
